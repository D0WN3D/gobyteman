...
git_dir = os.path.abspath(
    os.path.join(
        ...
        '..'))
gobyte_conf_dir = os.path.join(os.getenv('HOME'), '.gobytecore')
gobyte_cli_path = os.getenv('GoByte_CLI')
if os.getenv('GoByteMAN_PID') is None:
    quit("--> please run using 'gobyteman vote'")
...
def run_gobyte_cli_command(cmd):
    return run_command("%s %s" % (gobyte_cli_path or 'gobyte-cli', cmd))
...
def submit_votes(win, ballot, s):
    ...
    if votes_to_send.keys():
        if background_send:
            deferred_votes = tempfile.NamedTemporaryFile(prefix='sending_gobytevote_votes-',delete=False)
        ...
        for vote in sorted(votes_to_send, key=lambda s: s.lower()):
            ...
            for mn in sorted(masternodes):
                ...
                signature = gobyteutil.sign_vote(netvote, mnprivkey)
                command = ('%s' % gobyte_cli_path is not None and gobyte_cli_path or 'gobyte-cli') + ' voteraw ' + str(node['txid']) + ' ' + str(node['txout']) + ' ' + str(
...
def main(screen):
    ...
    if gobyte_cli_path is None:
        # test gobyte-cli in path -- TODO make robust
        try:
            run_command('gobyte-cli getinfo')
        except subprocess.CalledProcessError:
            quit(
                "--> cannot find gobyte-cli in $PATH\n" +
                "    do: export PATH=/path/to/gobyte-cli-folder:$PATH\n" +
                "    and try again\n")

    loadwin = curses.newwin(40, 40, 1, 2)

    loadwin.addstr(1, 2, 'gobytevote version: ' + version, C_CYAN)
    loadwin.addstr(2, 2, 'loading votes. please wait...', C_GREEN)
    loadwin.refresh()

    mncount = int(run_gobyte_cli_command('masternode count'))
    block_height = int(run_gobyte_cli_command('getblockcount'))
    days_to_next_cycle = (17280 - (block_height % 17280)) / 576.0
    days_to_finalization = days_to_next_cycle - 3

    # get ballot
    ballots = json.loads(run_gobyte_cli_command('gobject list all'))
    ballot = {}
    ...
    for entry in ballots:
        ...
        ballots[entry][u'votes'] = json.loads(run_gobyte_cli_command('gobject getvotes %s' % entry))
    ...
    masternodes = {}
    with open(os.path.join(gobyte_conf_dir, 'masternode.conf'), 'r') as f:
        lines = list(
            line
            for line in
            (l.strip() for l in f)
        ...
        for line in lines:
            ...
            masternodes[conf[3] + '-' + conf[4]] = {
                ...
                "txout": conf[4]}
    if not masternodes:
        # fallback to gobyte.conf entries if no masternode.conf entries
        with open(os.path.join(gobyte_conf_dir, 'gobyte.conf'), 'r') as f:
            lines = list(
                line
                for line in
                (l.strip() for l in f)
            ...
            if all(k in conf for k in ('masternode', 'externalip', 'masternodeprivkey')):
                # get funding tx from gobyteninja
                import urllib2
                mninfo = urllib2.urlopen(
                    "https://masternodes.gobyte.network/api/masternodes?ips=[\"" +
                    conf['externalip'] + ":12455" +
                    "\"]&portcheck=1").read()
                try:
                    mndata = json.loads(mninfo)
                    d = mndata[u'data'][0]
                except:
                    quit('cannot retrieve masternode info from gobyteninja')
                ...
                quit('cannot find masternode information in gobyte.conf')
    ...
    votewin.addstr(1, 2, 'gobytevote version: ' + version, C_CYAN)
    votewin.addstr(
        2,
        2,
