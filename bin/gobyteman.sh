...
# gobyteman - main executable
# installs, updates, and manages gobyte daemons and wallets
...
if [[ ${BASH_VERSION%%.*} != '4' ]];then
    die "gobyteman requires bash version 4. please update. exiting."
fi
...
GoByteMAN_BIN=$(readlink -f $0)
GoByteMAN_GITDIR=$(readlink -f ${GoByteMAN_BIN%%/bin/${GoByteMAN_BIN##*/}})
source $GoByteMAN_GITDIR/lib/gobyteman_functions.sh
...
source $GoByteMAN_GITDIR/lang/en_US.sh

# override if configured
lang_type=${LANG%%\.*}
[[ -e $GoByteMAN_GITDIR/lang/$lang_type.sh ]] && source $GoByteMAN_GITDIR/lang/$lang_type.sh

# process switch overrides ---------------------------------------------------

# show version and exit if requested
[[ $VERSION || $1 == 'version' ]] && echo $GoByteMAN_VERSION && exit 0
...
echo -e "${C_CYAN}${messages["gobyteman_version"]} $GoByteMAN_VERSION$GoByteMAN_CHECKOUT${C_NORM} - ${C_GREEN}$(date)${C_NORM}"
...
case "$1" in
        restart)
            ...
            # TODO, show uptime: ps --no-header -o pid,etime $(cat $INSTALL_DIR/gobyte.pid) | awk '{print $2}'
            case "$2" in
                now)
                    restart_gobyted
                    ;;
        ...
        sync)
            ...
            if [ -z $(git config user.email) ] ; then
                git config user.email "gobytemanuser"
                git config user.name "gobytemanuser"
            fi
            git stash
            git checkout master
            git reset --hard origin/master

            if [ -e $GoByteMAN_GITDIR/PREVIOUS_VERSION ]; then
                echo '--------------'
                cat_until "^$( cat $GoByteMAN_GITDIR/PREVIOUS_VERSION ) " $GoByteMAN_GITDIR/CHANGELOG.md | sed \
                ...
                echo '--------------'
                rm $GoByteMAN_GITDIR/PREVIOUS_VERSION
            fi

            if [ ! -z "$2" ]; then
                self=${0##*/};
                shift;
                exec $GoByteMAN_GITDIR/$self $@
            fi
        ...
        vote)
            ...
            ok " ${messages["done"]}"
            echo
            export GoByte_CLI GoByteMAN_PID=$$
            /usr/bin/env python $GoByteMAN_GITDIR/bin/gobytevote.py
            quit 'Exiting.'
            ;;
        status)
            COMMAND=$1
